...
struct PostTests {
 ...
 """
		...
		let viewPost = try ATJSONDecoder().decode(App.Bsky.Feed.Defs.FeedViewPost.self, from: Data(json.utf8))
...
"""
		...
		let postView = try ATJSONDecoder().decode(App.Bsky.Feed.Defs.PostView.self, from: Data(json.utf8))

		guard case let .post(post) = postView.record else { Issue.record(); return }

		#expect(post.langs == ["en"])
		#expect(post.text == "")

		guard case let .recordWithMediaView(mediaView) = postView.embed else { Issue.record(); return }

		guard case let .imagesView(imagesView) = mediaView.media else { Issue.record(); return }

		#expect(imagesView.images.count == 1)
		#expect(imagesView.images.first?.alt == "Bluesky Alt Text Rating\nHow consistently do you use alt text?\nMy score is 93%")
...
//			text: "hello",
//			embed: .embedRecord(
//				Bsky.Embed.Record(cid: "def", uri: "abc")
//			),
